...
export function deviceColor(device: Device): HslColor {
  ...
  if (device.kind === 'matrix' && device.tiles?.length) {
    const pixels = device.tiles.flatMap((tile) => tile.pixels);
...
export function deviceTypeLabel(device: Device): string {
  ...
  const pixels = device.tiles?.reduce((sum, tile) => sum + tile.pixels.length, 0) ?? 0;
  return `matrix · ${device.tiles?.length ?? 0} tiles · ${pixels}px`;
}
